...
docker rm -f solarservicemappingmultiproducer
docker run -it -d -e DISPLAY -e MAPPING_SERVICE_URL -e HOLOLENS_DATA_SET -e SOLAR_LOG_LEVEL -e "SERVICE_NAME=SolARServiceMappingMultiProducer" -v /tmp/.X11-unix:/tmp/.X11-unix --net=host --log-opt max-size=50m -e "SERVICE_TAGS=SolAR" --name solarservicemappingmultiproducer artwin/solar/services/mapping-multi-producer:latest
