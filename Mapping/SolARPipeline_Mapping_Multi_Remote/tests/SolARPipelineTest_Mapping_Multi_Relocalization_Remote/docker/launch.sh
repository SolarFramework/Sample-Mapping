...
docker run -it -d -e DISPLAY -e RELOCALIZATION_SERVICE_URL -e MAPPING_SERVICE_URL -e HOLOLENS_DATA_SET -e SOLAR_LOG_LEVEL -e "SERVICE_NAME=SolARPipelineMappingMultiRelocalizationClient" -v /tmp/.X11-unix:/tmp/.X11-unix --log-opt max-size=50m -e "SERVICE_TAGS=SolAR" --name solarpipelinemappingmultirelocalizationclient artwin/solar/pipeline/mapping-multi-relocalization-client:latest
